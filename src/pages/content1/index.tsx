...
const Users = () => {
  ...
  const onExporting = (e: { component: any }) => {
    ...
    exportDataGrid({
      ...
      workbook.xlsx.writeBuffer().then(function (buffer) {
        saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'content1.xlsx')
      })
    })
  }

  const getUserList = async () => {
    console.log(process.env.NEXT_PUBLIC_API_URL + 'content1/listApp')
    const response = await fetch(process.env.NEXT_PUBLIC_API_URL + 'content1/listApp', {
      method: 'GET'
    })
  ...
  const getDataOne = async (id: string, attributes: string) => {
    console.log(process.env.NEXT_PUBLIC_API_URL + 'content1/findData')
    const response = await fetch(
      process.env.NEXT_PUBLIC_API_URL + 'content1/findData?id=' + id + '&attributes=' + attributes,
      {
        method: 'GET'
      }
    )
  ...
  return (
    ...
    >
      ...
      <Column caption='게임명' calculateCellValue={() => '부산갈매기'} />
      <Column dataField='user_id' caption='유저 id' />

      <Column dataField='user.user_name' caption='유저이름' />
      ...
      {/* <Column
        ...
        caption='HMD 운동량'
        calculateCellValue={e => {
          if (e.hmd_calc == null) return ''
          const list = JSON.parse(e.hmd_calc)
          let total = 0.0
      ...
      {/* <Column
        ...
        caption='con 운동량'
        calculateCellValue={e => {
          if (e.con_calc == null) return ''
          const list = JSON.parse(e.con_calc)
          let total = 0.0
          list.map((element: string) => {
            const value = parseFloat(element)
            total += value
      ...
      {/* <Column
        ...
        caption='con2 운동량'
        calculateCellValue={e => {
          if (e.con2_calc == null) return ''
          const list = JSON.parse(e.con2_calc)
          let total = 0.0
          list.map((element: string) => {
            const value = parseFloat(element)
            total += value
