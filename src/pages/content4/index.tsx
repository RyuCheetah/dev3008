...
const Users = () => {
  ...
  const onExporting = (e: { component: any }) => {
    ...
    exportDataGrid({
      ...
      workbook.xlsx.writeBuffer().then(function (buffer) {
        saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'content4.xlsx')
      })
    })
  }

  const getUserList = async () => {
    console.log(process.env.NEXT_PUBLIC_API_URL + 'content4/listApp')
    const response = await fetch(process.env.NEXT_PUBLIC_API_URL + 'content4/listApp', {
      method: 'GET'
    })
  ...
  const getDataOne = async (id: string, attributes: string) => {
    console.log(process.env.NEXT_PUBLIC_API_URL + 'content4/findData')
    const response = await fetch(
      process.env.NEXT_PUBLIC_API_URL + 'content4/findData?id=' + id + '&attributes=' + attributes,
      {
        method: 'GET'
      }
    )
  ...
  return (
    ...
    >
      ...
      <Column caption='게임명' calculateCellValue={() => '달라진 러브하우스'} />
