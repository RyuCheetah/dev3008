...
const Users = () => {
  ...
  const getUserList = async () => {
    console.log(process.env.NEXT_PUBLIC_API_URL + 'content7/listApp')
    const response = await fetch(process.env.NEXT_PUBLIC_API_URL + 'content7/listApp', {
      method: 'GET'
    })
  ...
  const onExporting = (e: { component: any }) => {
    ...
    exportDataGrid({
      ...
      workbook.xlsx.writeBuffer().then(function (buffer) {
        saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'content7.xlsx')
      })
    })
  }
  ...
  const getDataOne = async (id: string, attributes: string) => {
    console.log(process.env.NEXT_PUBLIC_API_URL + 'content7/findData')
    const response = await fetch(
      process.env.NEXT_PUBLIC_API_URL + 'content7/findData?id=' + id + '&attributes=' + attributes,
      {
        method: 'GET'
      }
    )
  ...
  return (
    ...
    >
      ...
      <Column caption='게임명' calculateCellValue={() => '은행의 달인'} />
