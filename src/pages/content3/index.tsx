...
const Users = () => {
  ...
  const onExporting = (e: { component: any }) => {
    ...
    exportDataGrid({
      ...
      workbook.xlsx.writeBuffer().then(function (buffer) {
        saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'content3.xlsx')
      })
    })
  }

  const getUserList = async () => {
    console.log(process.env.NEXT_PUBLIC_API_URL + 'content3/listApp')
    const response = await fetch(process.env.NEXT_PUBLIC_API_URL + 'content3/listApp', {
      method: 'GET'
    })
  ...
  const getDataOne = async (id: string, attributes: string) => {
    console.log(process.env.NEXT_PUBLIC_API_URL + 'content3/findData')
    const response = await fetch(
      process.env.NEXT_PUBLIC_API_URL + 'content3/findData?id=' + id + '&attributes=' + attributes,
      {
        method: 'GET'
      }
    )
  ...
  return (
    ...
    >
      ...
      <Column caption='게임명' calculateCellValue={() => '맞춰봐요 러브하우스'} />
