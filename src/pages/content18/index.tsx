...
const Users = () => {
  ...
  const getUserList = async () => {
    console.log(process.env.NEXT_PUBLIC_API_URL + 'content18/listApp')
    const response = await fetch(process.env.NEXT_PUBLIC_API_URL + 'content18/listApp', {
      method: 'GET'
    })
  ...
  const getDataOne = async (id: string, attributes: string) => {
    console.log(process.env.NEXT_PUBLIC_API_URL + 'content18/findData')
    const response = await fetch(
      process.env.NEXT_PUBLIC_API_URL + 'content18/findData?id=' + id + '&attributes=' + attributes,
      {
        method: 'GET'
      }
    )
  ...
  return (
    ...
    >
      ...
      <Column caption='게임명' calculateCellValue={() => '틀린것을 찾아라'} />
