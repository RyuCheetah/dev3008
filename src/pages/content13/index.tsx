...
const Users = () => {
  ...
  const getUserList = async () => {
    console.log(process.env.NEXT_PUBLIC_API_URL + 'content13/listApp')
    const response = await fetch(process.env.NEXT_PUBLIC_API_URL + 'content13/listApp', {
      method: 'GET'
    })
  ...
  const getDataOne = async (id: string, attributes: string) => {
    console.log(process.env.NEXT_PUBLIC_API_URL + 'content13/findData')
    const response = await fetch(
      process.env.NEXT_PUBLIC_API_URL + 'content13/findData?id=' + id + '&attributes=' + attributes,
      {
        method: 'GET'
      }
    )
  ...
  return (
    ...
    >
      ...
      <Column caption='게임명' calculateCellValue={() => '신궁의 후예'} />
      ...
      <Column dataField='score' caption='성공률' />
      {/* <Column dataField='play_time' caption='play_time' /> */}
      <Column dataField='success_percentage' caption='score' />
      <Column
        dataField='play_time'
        caption='play_time'
