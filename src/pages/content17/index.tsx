...
const Users = () => {
  ...
  const getUserList = async () => {
    console.log(process.env.NEXT_PUBLIC_API_URL + 'content17/listApp')
    const response = await fetch(process.env.NEXT_PUBLIC_API_URL + 'content17/listApp', {
      method: 'GET'
    })
  ...
  const getDataOne = async (id: string, attributes: string) => {
    console.log(process.env.NEXT_PUBLIC_API_URL + 'content17/findData')
    const response = await fetch(
      process.env.NEXT_PUBLIC_API_URL + 'content17/findData?id=' + id + '&attributes=' + attributes,
      {
        method: 'GET'
      }
    )
  ...
  return (
    ...
    >
      ...
      <Column caption='게임명' calculateCellValue={() => '신나는 전국일주'} />
